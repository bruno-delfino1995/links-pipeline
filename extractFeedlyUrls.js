...
const filterContentUrls = R.filter(
  R.compose(
    i => i !== -1,
    R.indexOf("contents"),
    R.prop("url"),
...
const getContentUrls = R.map(
  R.compose(R.map(R.prop("href")), R.prop("alternate"))
);

const removeValueByKeyName = R.mapObjIndexed((v, k) =>
  /utm_*/.test(k) ? null : v
);
...
const removeTrackingParams = R.map(
  R.map(
    R.compose(
      ({ url, query, hash }) => {
